#!/usr/bin/env python3
"""
MLS English Word-Level Representation Analysis: Cross-Modal Minibatch CKA & PCA

Parallel to word_level_analysis.py (which uses LJSpeech) but uses a random
sample of 15,000 utterances from MLS English (OpenSLR-94 / facebook/multilingual_librispeech).

Pipeline
--------
  Stage 0 — Sample & download
      Streams the HuggingFace dataset (facebook/multilingual_librispeech, english)
      and saves exactly --n_utterances utterances (default 15,000) chosen via a
      reservoir sampler so the draw is uniformly random across the full train split
      without downloading the whole dataset.  Audio is saved as 16 kHz mono WAV;
      transcripts are saved in the plain-text format MFA expects (.lab files).

  Stage 1 — MFA forced alignment
      Runs  mfa align  on the downloaded corpus and produces one TextGrid per
      utterance.  MFA must already be installed and on PATH (conda activate mfa,
      or the mfa binary must be reachable).  The script uses the standard
      english_us_arpa acoustic model + dictionary; both are downloaded
      automatically by MFA if not already present.

  Stage 2 — Build word records
      Parses TextGrids (same parser as word_level_analysis.py) and builds
      word-level records with character offsets for text-model alignment.

  Stage 3 — Embed & analyse
      Extracts word-level embeddings from each model (Whisper enc/dec, Parakeet,
      Mimi, text LMs), then runs minibatch CKA and PCA — identical to the
      LJSpeech pipeline.

All outputs use the prefix  mls_  and live under  MLSData/  and  MLSPlots/
so nothing from the LJSpeech run is overwritten.

Usage
-----
  # Full pipeline (download + align + embed + analyse):
  python mls_word_level_analysis.py

  # Skip download if audio already present:
  python mls_word_level_analysis.py --mls_dir /path/to/mls_sample

  # Skip MFA if TextGrids already present:
  python mls_word_level_analysis.py --mls_dir /path/to/mls_sample --skip_mfa

  # Skip extraction if embeddings already cached:
  python mls_word_level_analysis.py --skip_extraction

  # Multi-run CKA for variability measurement:
  python mls_word_level_analysis.py --num_cka_runs 5

Dependencies (beyond the LJSpeech script requirements)
-------------------------------------------------------
  pip install datasets soundfile
  # MFA must already be installed (conda install -c conda-forge montreal-forced-aligner)
"""
...
MLS_SR           = 16000    # MLS audio is 16 kHz (after decode from opus/flac)
WHISPER_SR       = 16000
MIMI_SR          = 24000
...
# MFA settings — change these if you have a different pretrained model installed
MFA_ACOUSTIC_MODEL = "english_us_arpa"
MFA_DICTIONARY     = "english_us_arpa"
MFA_JOBS           = 4          # parallel MFA jobs; raise if you have many cores

# HuggingFace dataset identifier
# parler-tts/mls_eng is the streamable English-only MLS mirror.
# facebook/multilingual_librispeech does NOT include English as a config.
HF_DATASET_NAME   = "parler-tts/mls_eng"
HF_DATASET_CONFIG = None        # no config needed — dataset is English-only
HF_SPLIT          = "train"

# ---------------------------------------------------------------------------
# Model registry  (identical to word_level_analysis.py)
# ---------------------------------------------------------------------------
...
logger = logging.getLogger("mls_word_level")


def setup_logging(log_dir: Path) -> Path:
    log_dir.mkdir(parents=True, exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_path = log_dir / f"mls_run_{timestamp}.log"
...
# ---------------------------------------------------------------------------
# Stage 0 — Download MLS sample via HuggingFace streaming
# ---------------------------------------------------------------------------

def download_mls_sample(
    mls_dir: Path,
    n_utterances: int = 15_000,
    seed: int = 42,
) -> Path:
    """Stream MLS English train split and reservoir-sample n_utterances.

    Saves:
      <mls_dir>/wavs/<utt_id>.wav   — 16 kHz mono WAV
      <mls_dir>/lab/<utt_id>.lab    — plain-text transcript for MFA
      <mls_dir>/transcripts.json    — {utt_id: text} mapping
      <mls_dir>/metainfo.json       — {utt_id: {speaker_id, ...}}
    ...
    if transcripts_path.exists():
        ...
        if len(existing) >= n_utterances:
            logger.info(f"MLS sample already present ({len(existing):,} wavs) — skipping download")
            return mls_dir
        logger.info(f"Partial download found ({len(existing):,}/{n_utterances} wavs) — re-downloading")
    ...
    logger.info("Streaming MLS English train split from HuggingFace...")
    logger.info(f"  Target: {n_utterances:,} utterances (reservoir sample, seed={seed})")
    ...
    # parler-tts/mls_eng is plain Parquet — no trust_remote_code needed.
    # decode=False bypasses torchcodec/FFmpeg entirely; we decode with soundfile.
    load_kwargs: dict = dict(streaming=True)
    if HF_DATASET_CONFIG is not None:
        load_kwargs["name"] = HF_DATASET_CONFIG

    def _make_ds():
        ds = load_dataset(HF_DATASET_NAME, split=HF_SPLIT, **load_kwargs)
    ...
    # ------------------------------------------------------------------ #
    ds  = _make_ds()   # fresh iterator after the peek
    rng = np.random.default_rng(seed)
    reservoir: list = []
    pbar = tqdm(desc="Streaming MLS (reservoir sample)", unit="utt")
    ...
    for i, sample in enumerate(ds):
        ...
        # Stop after 10× the target — statistically well-mixed for 15k/500k.
        if i >= n_utterances * 10 - 1:
            logger.info(
                f"  Streamed {i+1:,} utterances — stopping early "
                f"({(i+1)//n_utterances}× target, reservoir well-mixed)"
            )
            break

    pbar.close()
    logger.info(f"Reservoir: {len(reservoir):,} utterances — saving to disk...")

    # ------------------------------------------------------------------ #
    # Save WAVs + .lab files                                              #
    # ------------------------------------------------------------------ #
    def _decode_audio(audio_field: dict) -> tuple[np.ndarray, int]:
        """Decode audio from a decode=False HF Audio field using soundfile."""
        raw = audio_field.get("bytes")
        if raw:
            array, sr = sf.read(_io.BytesIO(raw), dtype="float32", always_2d=False)
            return array, sr
        path = audio_field.get("path")
        if path:
            array, sr = sf.read(path, dtype="float32", always_2d=False)
            return array, sr
        raise ValueError(f"Audio field has neither bytes nor path: {list(audio_field.keys())}")
    ...
    for sample in tqdm(reservoir, desc="Saving WAVs + .lab files", unit="utt"):
        try:
            ...
            if sr != MLS_SR:
                ...
                t     = torchaudio.functional.resample(t, orig_freq=sr, new_freq=MLS_SR)
            ...
            metainfo[utt_id]    = {"speaker_id": speaker_id}

        except Exception as e:
            errors += 1
            logger.warning(f"Failed to save {sample.get('id', '?')}: {e}", exc_info=True)

    if errors:
        logger.warning(f"{errors} samples failed to save")
    if not transcripts:
        raise RuntimeError(
            "No utterances were saved — all samples failed.\n"
            "Check the warnings above for the root cause."
        )

    with open(transcripts_path, "w") as f:
        json.dump(transcripts, f, indent=2)
    with open(mls_dir / "metainfo.json", "w") as f:
        json.dump(metainfo, f, indent=2)

    n_saved = len(transcripts)
    logger.info(f"Saved {n_saved:,} utterances  →  {mls_dir}")
    logger.info(f"  WAVs:        {wavs_dir}")
    logger.info(f"  .lab files:  {lab_dir}")
...
# ---------------------------------------------------------------------------
# Load MLS utterances (parallel to load_ljspeech)
# ---------------------------------------------------------------------------

def load_mls_utterances(mls_dir: Path) -> dict:
    """Load pre-downloaded MLS sample.

    Returns dict: utt_id → {audio: np.ndarray, sr: int, text: str}
    Audio is loaded lazily on first access to keep memory usage reasonable;
    here we just record paths and transcripts.  For compatibility with the
    extraction functions (which expect audio arrays), we load everything
    eagerly — the 15k utterances at ~5 s each = ~75k s ≈ 1.2 GB float32.
    """
    transcripts_path = mls_dir / "transcripts.json"
    wavs_dir         = mls_dir / "wavs"
    ...
    if not transcripts_path.exists():
        raise FileNotFoundError(
            f"transcripts.json not found in {mls_dir}. "
    ...
    logger.info(f"Loading {len(transcripts):,} MLS utterances from {mls_dir}...")
    utterances = {}

    for utt_id, text in tqdm(transcripts.items(), desc="Loading MLS audio", unit="utt"):
        wav_path = wavs_dir / f"{utt_id}.wav"
        if not wav_path.exists():
            logger.warning(f"WAV missing for {utt_id}")
            continue
    ...
    logger.info(f"Loaded {len(utterances):,} MLS utterances")
    return utterances


# ---------------------------------------------------------------------------
# TextGrid parser  (identical to word_level_analysis.py, copied here for
# self-containment; handles both "words"/"word" tier names and also the
# MFA speaker-named tiers like "1234 - words")
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Build word records  (same logic as word_level_analysis.py)
# ---------------------------------------------------------------------------
...
def build_word_records(
    ...
    TextGrids from MFA may live in speaker subdirectories under textgrid_dir.
...
# ---------------------------------------------------------------------------
# Checkpoint helpers  (identical to word_level_analysis.py)
# ---------------------------------------------------------------------------
...
def extract_whisper_enc_word_embeddings(
    ...
    checkpoint_path = checkpoint_dir / f"mls_{model_name}_checkpoint.pkl" if checkpoint_dir else None
...
# ---------------------------------------------------------------------------
# Minibatch CKA  (identical to word_level_analysis.py)
# ---------------------------------------------------------------------------

def _hsic1_batch(X, Y):
    n  = X.shape[0]
    assert n >= 4
    K  = X @ X.T;  L  = Y @ Y.T
...
def minibatch_cka(X, Y, batch_size=MINIBATCH_SIZE, seed=MINIBATCH_SEED):
    ...
    for start in range(0, N - batch_size + 1, batch_size):
        ...
        Xb  = X[idx].astype(np.float64);  Yb = Y[idx].astype(np.float64)
...
def run_cka_multi(embeddings, names, batch_size, num_runs, base_seed=MINIBATCH_SEED):
    n       = len(names)
    n_pairs = n * (n - 1) // 2
...
# ---------------------------------------------------------------------------
# Plots  (same as word_level_analysis.py, all filenames prefixed with mls_)
# ---------------------------------------------------------------------------
...
def _off_diagonal_sims(embeddings, max_n=3000, seed=42, normalize=False):
    rng = np.random.default_rng(seed)
    sims = {}
...
def _plot_sim_suite(sims, hist_dir, prefix, file_tag, xlabel, overlay_xlim, color_map, log_fn):
...
def plot_similarity_histograms(embeddings, plots_dir, prefix="mls_", max_n=3000, seed=42):
    """Per-model, faceted, overlay, mean, and SD plots for dot-product and cosine similarity."""
    _apply_style()
    ...
    _plot_sim_suite(_off_diagonal_sims(embeddings, max_n=max_n, seed=seed),
                    hist_dir, prefix, "dot_product_sim", "Dot Product Similarity",
                    (-10, 10), color_map, logger.info)
    _plot_sim_suite(_off_diagonal_sims(embeddings, max_n=max_n, seed=seed, normalize=True),
                    hist_dir, prefix, "cosine_sim", "Cosine Similarity",
                    (-1, 1), color_map, logger.info)
...
def plot_cka_heatmap(cka_matrix, names, plots_dir, ci95_matrix=None,
                     filename="mls_word_cka_heatmap.png"):
    _apply_style()
...
def plot_cka_multi_run(mean_matrix, ci95_matrix, per_run_matrices, names, plots_dir, num_runs):
    ...
    if num_runs <= 6:
        ...
        _draw_cka_heatmap_axes(ax_mean, mean_matrix, names, ci95_matrix=ci95_matrix,
                               title=f"MLS Mean CKA across {num_runs} shuffles\n(±95% CI across runs)")
    ...
    plt.tight_layout()
    path = plots_dir / "mls_word_cka_heatmap_multi_run.png"
    plt.savefig(path, dpi=150, bbox_inches="tight")
    plt.close()
    logger.info(f"  Saved → {path}")
...
def plot_cka_variability_scatter(results_dict, names, plots_dir):
    ...
    ax.set_title("MLS CKA Estimate Stability", fontsize=12, fontweight="bold")
    ax.set_xlim(-0.02, 1.02); ax.set_ylim(bottom=0)
    plt.tight_layout()
    path = plots_dir / "mls_word_cka_variability_scatter.png"
    plt.savefig(path, dpi=150, bbox_inches="tight")
    plt.close()
    logger.info(f"  Saved → {path}")
...
def plot_eigenspectra_overlay(eigenvalues, plots_dir, max_pc=PCA_PLOT_MAX_PC):
    ...
    ax.set_title(f"MLS Word-Level PCA Eigenvalue Spectra  ·  first {max_pc} PCs",
                 fontsize=12, fontweight="bold")
    plt.tight_layout()
    path = plots_dir / "mls_word_eigenspectra_overlay.png"
    plt.savefig(path, dpi=150, bbox_inches="tight")
    plt.close()
    logger.info(f"  Saved → {path}")
...
def plot_effective_rank_bar(eigenvalues, plots_dir):
    ...
    ax.set_title("MLS Word-Level Effective Rank per Model", fontsize=12, fontweight="bold")
    for bar, rank in zip(bars, ranks):
        ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.3,
                f"{rank:.1f}", ha="center", va="bottom", fontsize=8)
    plt.tight_layout()
    path = plots_dir / "mls_word_effective_rank.png"
    plt.savefig(path, dpi=150, bbox_inches="tight")
    plt.close()
    logger.info(f"  Saved → {path}")
...
def save_summary_tables(mean_matrix, ci95_matrix, results_dict, names,
    ...
    if per_run_matrices:
        for r, mat in enumerate(per_run_matrices):
            path_r = data_dir / f"mls_word_cka_matrix_run{r+1}.csv"
    ...
    path2 = data_dir / "mls_word_cka_stats.csv"
    ...
    path3 = data_dir / "mls_word_pca_summary.csv"
...
def parse_args():
    p = argparse.ArgumentParser(
        description="MLS English word-level cross-modal CKA (download → MFA → embed → analyse)"
    )
    p.add_argument("--root_dir", default=".", type=Path,
                   help="Root directory for MLSData/, MLSPlots/, logs/")
    p.add_argument("--mls_dir", default=None, type=Path,
                   help="Path to an existing MLS sample directory (wavs/ + transcripts.json). "
                        "If omitted, defaults to <root_dir>/MLSData/mls_sample and downloads.")
    p.add_argument("--textgrid_dir", default=None, type=Path,
                   help="Path to an existing TextGrid directory from MFA. "
                        "If omitted, defaults to <root_dir>/MLSData/textgrids and runs MFA.")
    p.add_argument("--n_utterances", default=15_000, type=int,
                   help="Number of utterances to sample from MLS (default: 15000)")
    p.add_argument("--sample_seed", default=42, type=int,
                   help="Random seed for reservoir sampling")
    p.add_argument("--skip_download", action="store_true",
                   help="Skip download stage (mls_dir must already exist)")
    p.add_argument("--skip_mfa", action="store_true",
                   help="Skip MFA stage (textgrid_dir must already exist)")
...
def main():
    ...
    data_dir  = root / "MLSData"
    plots_dir = root / "MLSPlots"
    data_dir.mkdir(parents=True, exist_ok=True)
    plots_dir.mkdir(parents=True, exist_ok=True)

    log_path = setup_logging(root / "logs")
    logger.info("=" * 60)
    logger.info("MLS Word-Level Representation Analysis — run started")
    ...
    mls_dir      = args.mls_dir      or (data_dir / "mls_sample")
    ...
    if not args.skip_download:
        with timer("Download MLS sample"):
            download_mls_sample(mls_dir, n_utterances=args.n_utterances,
                                seed=args.sample_seed)
    else:
        logger.info(f"--skip_download: using existing MLS sample at {mls_dir}")
    ...
    # ------------------------------------------------------------------
    with timer("Load MLS utterances"):
        utterances = load_mls_utterances(mls_dir)

    cache_suffix      = f"_max{args.max_words}" if args.max_words else ""
    word_records_path = data_dir / f"mls_word_records{cache_suffix}.json"
    ...
    for model_name, cfg in MODELS.items():
        cache_path = data_dir / f"mls_word_embeddings_{model_name}.pkl"
    ...
    logger.info("=" * 60)
    logger.info(f"Pairwise MLS word-level CKA  "
    ...
    with timer(f"Pairwise CKA ({num_runs} run(s))"):
        mean_matrix, ci95_matrix, cka_results = run_cka_multi(
            embeddings, names, batch_size=args.batch_size,
    ...
    results_json = {
        "dataset":          "MLS English (OpenSLR-94)",
    ...
    json_path = data_dir / "mls_word_results.json"
