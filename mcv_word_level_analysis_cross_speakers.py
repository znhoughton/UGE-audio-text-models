...
def build_word_type_ids(word_records: list) -> np.ndarray:
    ...
    """
    # Key: (word, sentence, char_start) — char_start is derived from the text
    # so it is identical for the same word position across all speakers,
    # correctly grouping cross-speaker tokens of the same word together.
    all_types: dict = {}
    ids = []
...
def plot_similarity_histograms(embeddings, plots_dir, prefix="mcv_", max_n=3000, seed=42, word_ids=None):
    """Per-model, faceted, and overlay similarity histograms (dot product and cosine)."""
    _apply_style()
...
def main():
    ...
    with timer("Similarity histograms"):
        plot_similarity_histograms(embeddings, plots_dir, prefix="mcv_", word_ids=word_ids)
